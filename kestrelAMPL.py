...
class kestrelAMPL:
    ...
    def __init__(self):
        ...
        if not self.email:
            sys.stdout.write(
                "An email address is required for NEOS submissions.\n")
    ...
    def submit(self, xml):
        user = "{} on {}".format(os.getenv('LOGNAME'),
                                 socket.getfqdn(socket.gethostname()))
        if (self.username is None or self.userpassword is None):
            (jobNumber, password) = self.neos.submitJob(xml, user, "kestrel")
        else:
            (jobNumber, password) = self.neos.authenticatedSubmitJob(
                xml, self.username, self.userpassword, "kestrel")
        ...
        if self.options is not None:
            ...
            if m:
                ...
                if not NEOS_solver_name:
                    sys.stdout.write(
                        "{} is not available on NEOS.  Choose from:\n".format(solver_name))
                    for s in kestrelAmplSolvers:
                        sys.stdout.write("\t{}\n".format(s))
                    sys.stdout.write(
